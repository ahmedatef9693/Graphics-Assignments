...
void Renderer::Initialize()
{
	...
	matrixid = glGetUniformLocation(programID, "ModelMatrix"); //model matrix in  vertex shader
	glm::mat4 scalemat = glm::scale(0.25f, 0.25f, 0.0f);
...
void Renderer::Draw()
{
		...
		glVertexAttribPointer(
		    1,                  // attribute 0. No particular reason for 0, but must match the layout in the shader.1
			3,                  // size
			GL_FLOAT,           // type
			GL_FALSE,           // normalized?
			sizeof(float)*6,
			(char*)(sizeof(float)*3)                  // stride
			           // array buffer offset
			);
